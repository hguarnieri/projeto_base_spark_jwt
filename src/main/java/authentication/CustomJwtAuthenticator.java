 ...
 * @since 1.8.0
 */
public class CustomJwtAuthenticator implements TokenAuthenticator {
    ...
    public CustomJwtAuthenticator(final String signingSecret) {
     ...
     * @since 1.8.2
     */
    public CustomJwtAuthenticator(final String signingSecret, final String encryptionSecret) {
        this.signingSecret = signingSecret;
    ...
    private static void createJwtProfile(final TokenCredentials credentials, final SignedJWT signedJWT) throws ParseException {
        ...
        //TODO: Criar uma função que verifica se o Token ainda é valido

        // Cria objetos a serem usados
        final UserProfile profile = new HttpProfile();
        UserDAO dao = new UserDAO();

        // Busca informações do usuário
        User user = dao.getUser((String) claims.get(CommonProfile.USERNAME));

        // Define as informações do usuário no perfil
        profile.setId(user.getUsername());
        profile.addAttribute(CommonProfile.USERNAME, JwtProfile.class.getSimpleName());
        profile.addAttribute("token", credentials.getToken());
        profile.addRoles(user.getRoles());

        // Define o novo perfil
